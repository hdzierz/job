...
function message_send($message_id,$check,$type){
	global $MYSQL;
	global $MESSAGE;
	...
	$res_con = query($qry,0);
	$con = mysql_fetch_object($res_con);
	if(count($check)==0) return;
	//$now=date("Y_m_d_h_i_s");
	
	foreach($check as $id=>$on){
	if(!$on) continue;
		//send_operator_simple_mail($type,$id,$con->title,$con->message,"0211534438@sms.tnz.co.nz");
		send_operator_simple_mail($type,$id,$con->title,$con->message,false);	
		
	}// while($dist = mysql_fetch_object($res_dist))
	
	$MESSAGE = "Sending Job details finished.<br />";
}
...
function show_do_details_send($job_id,$choice){
	...
	$dir = $SEND_OUTPUT_DIR."temp_do_det/".$now;
