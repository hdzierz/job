...
function create_job_no(){
	$last_job_no=get_max("job_temp","job_no","WHERE job_no<10000","");
	...
	if(!$last_job_no){
		$last_job_no=get_max("job_temp","job_no","","");
		if(!$last_job_no){
			$last_job_no=10000;
		}
	}
...
function write_header_without_contact($job_id,$title){
	$qry = "SELECT * FROM job_temp
			LEFT JOIN client
			ON job_temp.client_id=client.client_id
			WHERE job_temp.job_id=$job_id";
...
function collect_comment($dist,$job_id){
	$show_comments = get("job_temp","show_comments","WHERE job_id='$job_id'");
	
	if($show_comments=='Y'){
		$comment = get("job_temp","comments","WHERE job_id='$job_id'");
	}
	else{
		$comment = "";
	}
	...
	$qry = "SELECT comments FROM job_route_temp 
			LEFT JOIN route
...
function show_do_details($job_id,$export=0){
	...
	$qry = "SELECT DISTINCT job_route_temp.dist_id,company 
			FROM job_route_temp
			LEFT JOIN route
			ON route.route_id=job_route_temp.route_id
			LEFT JOIN operator
			ON operator.operator_id=job_route_temp.dist_id
			WHERE job_route_temp.job_id='$job_id'
			AND job_route_temp.dist_id IS NOT NULL 
	...
	while($dist = mysql_fetch_object($res_dist)){
		...
		$qry = "SELECT DISTINCT region 
				FROM route 
				LEFT JOIN job_route_temp
				ON job_route_temp.route_id=route.route_id
				WHERE job_route_temp.dist_id=$dist->dist_id 
				ORDER BY island,seq_region,seq_area";
		$res_reg = query($qry,0);
		$start=true;
		while($region = mysql_fetch_object($res_reg)){
			...
			$qry = "SELECT  IF(operator.alias<>'',operator.alias,
					...
					WHERE job_route_temp.job_id='$job_id'
							AND job_route_temp.dist_id='$dist->dist_id'
							AND route.region = '$region->region'
					GROUP BY route.region,job_route_temp.doff,job_route_temp.version
					ORDER BY island,route.seq_region,company";
			//echo nl2br($qry)."<br />";
			$res_t = query($qry);
...
function show_job_details($job_id,$export=false,$choice){
	...
	$comments = get("job_temp","comments","WHERE job_id='$job_id'");
	...
	while($dist = mysql_fetch_object($res_dist)){
			...
			while($do = mysql_fetch_object($res_do)){
				...
				$qry = "SELECT  CONCAT(IF(operator.alias<>'',operator.alias,
						...
						WHERE job_route_temp.job_id='$job_id'
								...
								$where_add
						ORDER BY island,route.seq_region,route.seq_area,route.seq_code,job_route_temp.dest_type,company";
				//echo nl2br($qry)."<br />";
				$res_t = query($qry);
				//echo ";<br />";
				...
				if(mysql_num_rows($res_t)>0){
					if($start){
						write_header_without_contact($job_id,"COURAL JOB DETAILS");
						$tab->startTable();
						$tab->startNewLine();
							$tab->addLineWithStyle("Distributor: ".$dist->company,"sql_extra_line_text10",3);
						$tab->stopNewLine();			
						$has_lines=true;
					}
					...
					$tab->addLine("");
					$tab->addLine("DO:");
					$qry = "SELECT SUM(amount) AS sum FROM job_route_temp LEFT JOIN route ON route.route_id=job_route_temp.route_id WHERE dist_id='$dist->dist_id' AND job_route_temp.job_id='$job_id' and dest_type='bundles' AND job_route_temp.doff = '$do->doff' $where_add GROUP BY job_id,dist_id ";
					$res_sum = query($qry);
					$sum = mysql_fetch_object($res_sum);
					$total = $sum->sum;
					if(!$total) $total=0;
					...
					$tab->addLine("$total");
					if($alt_job_id){
						$qry = "SELECT SUM(amount) AS sum FROM job_route_temp LEFT JOIN route ON route.route_id=job_route_temp.route_id WHERE dist_id='$dist->dist_id' AND job_route_temp.job_id='$alt_job_id' and dest_type<>'bundles'  AND region='$region->region' AND job_route_temp.doff = '$do->doff' $where_add GROUP BY job_id,dist_id ";
						$res_sum = query($qry);
						$sum = mysql_fetch_object($res_sum);
						$total = $sum->sum;
						if(!$total) $total=0;
		...
		if($has_lines){
			$tab->startNewLine();
				...
				$qry = "SELECT SUM(amount) AS sum FROM job_route_temp LEFT JOIN route ON route.route_id=job_route_temp.route_id WHERE dist_id='$dist->dist_id' AND job_route_temp.job_id='$job_id' and dest_type='bundles'  $where_add GROUP BY job_id,dist_id ";
				$res_sum = query($qry);
				$sum = mysql_fetch_object($res_sum);
				$total = $sum->sum;
				if(!$total) $total=0;
...
function show_print_table_check_export($job_id,$export=false){
	set_alt_do_contractors($job_id);
	write_header($job_id,"COURAL RD CHECK");
											...
											IF(job_route_temp.dest_type='num_sheepbeef','Sheep/Beef',
												IF(job_route_temp.dest_type='num_dairybeef','Dairy/Beef',
													IF(job_route_temp.dest_type='num_hort','Hort',
														IF(job_route_temp.dest_type='num_nzfw','F@90%',
...
function show_table($job_id){
	...
	if($alt_job_id){
		...
		$add_sel = "IF(jra.dest_type<>'bundles',jra.amount,0) AS 'Add-Circ',";
		$add = "LEFT JOIN job_route_temp jra
				ON jra.job_id = job_temp.alt_job_id AND jra.route_id = job_route_temp.route_id";
	}
	...
	$qry = "SELECT 	job_route_temp.job_route_id 	AS Record,
					...
					IF(job_route_temp.dest_type='bundles',job_route_temp.amount,0) AS 'Bdls',
					$add_bdl
					IF(job_route_temp.dest_type<>'bundles',job_route_temp.orig_amt,'0')			AS 'Orig-Circ',
					IF(job_route_temp.dest_type<>'bundles',job_route_temp.amount,0) AS 'Circ',
					$add_sel
					job_route_temp.version			AS Version,
