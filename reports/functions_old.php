...
function write_bible($date,$dist_id,$mode,$home_phone,$mobile_phone){
...
*/
		...
		$qry = "SELECT operator_id as dist_id FROM operator WHERE is_dist='Y' and company not like 'Reach Media%'";
		if($dist_id>0){
		...
		while($dist=mysql_fetch_object($res_dist)){
			...
			if(strpos($dist_o->company,"LBC")===false){
				...
				$tab->colWidth["Address"]=150;
				$tab->colWidth["Phone"]=150;
				$tab->colWidth["Parcel Drop Off"]=150;
				...
				//$subdist = mysql_fetch_object($res_sd)	;
					...
					if ($subdist->name2 != "" || $subdist->first_name2 != "")
					{
						$sd_name .= "<br />".$subdist->name2.", ".$subdist->first_name2;
					}
					...
					$qry = "SELECT  GROUP_CONCAT(DISTINCT RD ORDER BY RD SEPARATOR ' /<br />')
								...
								SELECT 	  CONCAT(
											  ...
											  GROUP_CONCAT(DISTINCT code_rd ORDER BY code SEPARATOR ',')
																	...
																	AS RD,
										   IF(first_name2 <> '' OR name2 <> '',CONCAT(name,', ',first_name, '<br />', name2,', ', first_name2),CONCAT(name,', ',first_name)) 
																	AS Name,
		...
		<?								
					...
					*/
					...
					if($subdist_start){
						$tab->startNewLine();
							$tab->addLineWithStyle("<div style='float:right; font-size:8pt; font-weight:bold '>Page: $page_num</div>","sql_extra_line_text",8);
						$tab->stopNewLine();
						$page_num++;
						$tab->startNewLine();
				...
				while($subdist = mysql_fetch_object($res_sd)){
					...
					if($mode=="xls"){
						...
						$qry = "SELECT 	       code						AS RD,
											   CONCAT_WS(', ',address.name,address.first_name) 
																		AS Name,
												IF(
													operator.env_deliv_notes IS NOT NULL AND operator.env_deliv_notes<>'',
													CONCAT(CONCAT_WS(', ',operator.do_address,operator.do_city),
														'Notes: ',
														operator.env_deliv_notes),
													CONCAT_WS(', ',operator.do_address,operator.do_city)) 
																		AS Address,
											   $phone
										...
										WHERE '$date' BETWEEN route_aff.app_date AND route_aff.stop_date
											/*app_date = (SELECT MAX(app_date) FROM route_aff WHERE route_id=route.route_id)*/
											AND route_aff.env_subdist_id='$subdist->subdist_id'
											AND route_aff.env_dist_id='$dist->dist_id'
											/*AND route.is_hidden<>'Y'*/
										ORDER BY island,seq_region,seq_area,seq_code";
...
<?								
	...
	function write_geo_table($date,$regions,$islands,$target,$mode,$home_phone,$mobile_phone){
		...
		if($regions[0]=='0'){
			...
			$qry = "SELECT DISTINCT region from route 
					...
					AND region not like '%ZZ%' 
					AND region not like '%REACH%' 
		...
		foreach($regions as $region){
			...
			*/
			...
			//new sizing after description and parcel dropoff fields were added 3/8/09 JW
			$tab->colWidth["RD"]=150;
			$tab->colWidth["Name"]=150;
			$tab->colWidth["Address"]=150;
			$tab->colWidth["Phone"]=150;
			...
			while($dist = mysql_fetch_object($dist_res)){
				...
				$qry = "SELECT 	operator.operator_id AS subdist_id,
						...
						LEFT JOIN address
						ON address.operator_id=operator.operator_id
						WHERE route_aff.env_dist_id='$dist->operator_id'
							AND '$date' BETWEEN route_aff.app_date AND route_aff.stop_date
						GROUP BY operator_id
						ORDER BY subdist_seq,island,seq_region,seq_area";
				$res_sd = query($qry,0);
				...
				while($subdist=mysql_fetch_object($res_sd)){				
					...
					/*if($mode=="screen"){
							$view = ", CONCAT('<a target=\'_blank\' href=\'http://www.google.com/maps?daddr=',do_address,' ',do_city,' New Zealand','\'>View on Map</a>')
																	AS View";
					...
					if($mode=="xls"){
						...
						$qry2 = "SELECT 		code						AS RD,
										...
										WHERE '$now' BETWEEN route_aff.app_date AND route_aff.stop_date
											...
											AND region='$region'
											AND route_aff.env_subdist_id='$subdist->operator_id'
											AND route.is_hidden<>'Y'
										ORDER BY island,seq_region,seq_area,seq_code,RD";
					}	
					else if($mode=="print" || $mode=="screen"){
						...
						$qry2 = "SELECT  GROUP_CONCAT(DISTINCT RD ORDER BY RD SEPARATOR ' /<br />')
										...
										SELECT 	  CONCAT(
												   ...
												   CONCAT_WS(', ',name,first_name) 
																			AS Name,
												   TRIM(CONCAT_WS(', ',IF(address='','',address),IF(city='','',city))) 				
																			AS Address,
												   $phone
	...
	<?			
						...
						//$tab->startNewLine();
						//	$header++;
						//	$tab->addLineWithStyle("S/Dist.: $sd_name,  Phone: $sdphone,  Mobile: $sdmobile","sql_extra_line_text_grey_small",8);
						//$tab->stopNewLine();
