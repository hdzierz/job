...
function write_dist_table($date,$dist_id,$mode,$home_phone,$mobile_phone){
	...
	while($dist=mysql_fetch_object($res_dist)){
		...
		$dist_email="none";
		if($dist_o->email) $dist_email=$dist_o->email;
		...
		if(strpos($dist_o->company,"LBC")===false){
			...
			$tab->addLineWithStyle("<div style='float:right; font-size:20pt; font-weight:bold '>$dist_country</div>
														...
														$dist_name<br />
														  Phone: $dist_phone<br />
														  Mobile: $dist_mobile<br />Email: $dist_email","sql_extra_line_text_grey",8);
			$tab->stopNewLine();
			...
			while($subdist = mysql_fetch_object($res_sd)){
				...
				$sdemail="none";
				if($subdist->email) $sdemail = $subdist->email;
...
<div class="pagebreak">&nbsp;</div>
															...
															$tab->startNewLine();
															$tab->addLineWithStyle("S/Dist.: $sd_name,  Phone: $sdphone,  Mobile: $sdmobile, Email: $sdemail","sql_extra_line_text_grey",8);
