...
AutoSuggestControl.prototype.createDropDown = function () {
    ...
    this.layer.className = "suggestions_2";
...
AutoSuggestControl.prototype.handleKeyUp = function (oEvent /*:Event*/) {
    ...
    if (iKeyCode == 8 || iKeyCode == 46) {
        ...
        this.timeoutId = setTimeout( function () {
            oThis.provider.requestSuggestions(oThis, false, oPath,deb	);
        }, 250);
    ...
    //make sure not to interfere with non-character keys
        ...
        this.timeoutId = setTimeout( function () {
            oThis.provider.requestSuggestions(oThis, true, oPath,deb);
        }, 250);
    }
};
 ...
 * @scope public
 */
function SuggestionProvider(tableName, fieldName, tField) {
    this.http = zXmlHttp.createRequest();
	this.tableName = tableName;
	this.fieldName = fieldName;
	this.tfieldid  = document.getElementById(tField).id;
...
SuggestionProvider.prototype.requestSuggestions = function (oAutoSuggestControl /*:AutoSuggestControl*/,
    ...
    var oData = { 
       // requesting: "StatesAndProvinces", 
		//fieldname: "Name",
		requesting: this.tableName, 
		fieldname: this.fieldName,
		tfieldid: this.tfieldid,
        text: oAutoSuggestControl.userText,
        limit: 10 
    };
	 
	var completePath = "" + path + "suggestions.php";
	
	    //open connection to server
    oHttp.open("post", completePath, true);
    ...
    oHttp.onreadystatechange = function () {
        if (oHttp.readyState == 4) {
			if (oHttp.status == 200) {
				...
				if(debug==true) {alert(oHttp.responseText);}
				var aSuggestions = JSON.parse(oHttp.responseText);
	
				//provide suggestions to the control
				oAutoSuggestControl.autosuggest(aSuggestions, bTypeAhead);             
			} else {
				alert('There was a problem with the request. State' + oHttp.readyState + ' Status: '+oHttp.status);
			}
        } 
    };
