...
function turn_fax($dir,$file){
	...
	$infile = 'C:\\Program Files\\Apache Software Foundation\\Apache2.2\\htdocs\\'.$dir.'/'.$file;
	$outfile = 'C:\\Program Files\\Apache Software Foundation\\Apache2.2\\htdocs\\'.$dir.'/'.$bfile.'tr.pdf';
	...
	//echo "cmd /C pdftk \"$infile\" cat 1-endE output \"$outfile\"> \"C:\\temp\\tt.txt\"";
	if($oExec==0){
		return $outfile;
	}
	return false;
...
function send_operator_mail($target,$dir,$file,$id,$email=false){
	...
	if($email){
		...
		if($alt_email)
		{
			...
			if(strpbrk($alt_email,'@') == "@".$FAX_EMAIL_ADDRESS)
			{
