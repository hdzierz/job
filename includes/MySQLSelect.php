...
?>
			<option <? if(in_array($this->optionDefValue,$this->optionIsVal)){?> selected <? }?> value="<?=$this->optionDefValue?>"><?=$this->optionDefText?></option>
...
<?
			while($obj = mysql_fetch_object($res)){
				if(strpos($obj->id,'&')){
					...
					$id =$obj->id;
?>
				<option <? if(in_array($id,$this->optionIsVal)){?> selected <? }?> value="<?=$id?>"><?=$obj->name?></option>
<?
			}
...
<?		
	...
	function writeSelect(){
		$qry = "SELECT DISTINCT $this->SQLField AS name, 
