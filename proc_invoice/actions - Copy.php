...
function write_header($fp){
	...
	fwrite($fp, "Invoice  No.\tDate\tCustomer PO\tDelivery Status\tItem Number\tQuantity\tDescription\tPrice\tInc-GST Price\tTotal\tJournal Memo\tGST Code\tGST Amount\tSale Status\tCard ID\n");
}
...
//echo "SUM((folding_fee+rate)*invoice_qty*(1-discount/100))","WHERE invoice_no='$invoiceno'";
	...
	$total 				= number_format($qty*$price,3,'.','');
	//echo $fuel_surcharge." ".$invoiceno." ".$job->job_no."<br />";
	$total_gst			= number_format($total*$gst,3,'.','');
	...
	if($total>0) 
		...
		fwrite($fp, "$invoiceno\t$date\t$cust_po\t$delivery_status\t$item_no\t$qty\t$description\t$price\t$price_gst\t$total\t$journal\t$gst_code\t$gst\t$sale_stat\t$card_id\n");
}
function write_bundles($fp,$job,$invoiceno,$fuel_surcharge,$gst,$date){
	...
	while($amt = mysql_fetch_object($res)){
		...
		fwrite($fp, "$invoiceno\t$date\t$cust_po\t$delivery_status\t$item_no\t$qty\t$description\t$price\t$price_gst\t$total\t$journal\t$gst_code\t$gst_val\t$sale_stat\t$card_id\n");
	}	
}
...
if($action=="Create Invoices"){
	...
	if($check){
		...
		while($job = mysql_fetch_object($res)){
			...
			$price_gst			= number_format($price*$gst,4,'.','');
			$total 				= number_format($job->invoice_qty*$price,3,'.','');
			$total_gst			= number_format($total*$gst,3,'.','');
			$gst_val			= number_format($total_gst - $total,2,'.','');
			$gst_code			= 'S';
			$journal			= $job->publication;
