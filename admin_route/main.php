...
if($action=="edit" || $action=="add"){
		...
		$sql = "SELECT COUNT(*) AS ct FROM job_route WHERE job_route.route_id=$record GROUP BY job_id";
		$ct_route = mysql_fetch_object(query($sql));
		$ct_route = $ct_route->ct;
		...
		if(!$ct_route) $ct_route=0;							
?>	
		<form name="editroute" action="admin_route.php?action=save" method="get">
			<table>
				<tr>
					<td colspan="3">Route used in <?php echo $ct_route; ?> jobs<br /><br /></td>
				</tr>
			  <tr>
					<td>Island</td>
					<td>
...
if($action=="" || !isset($action)){
	$now = date("Y-m-d");
	$where=" WHERE '$now' BETWEEN app_date AND stop_date";
